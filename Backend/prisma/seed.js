...
async function main() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await prisma.interviewAnalysis.create({
        data: {
          userAnswer: `Answer ${i + 1} by Tony Stark...`,
          feedback: `Feedback for answer ${i + 1}`,
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await prisma.interviewAnalysis.create({
        data: {
          userAnswer: `Answer ${i + 1} by Natasha...`,
          feedback: `Feedback for answer ${i + 1}`,
          scorePerQuestion: randomScore(),
          interviewId: interview.interviewId,
          videoQuestionId: question.videoQuestionId
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await prisma.interviewAnalysis.create({
        data: {
          userAnswer: `Answer ${i + 1} by Bruce...`,
          feedback: `Feedback for answer ${i + 1}`,
          scorePerQuestion: randomScore(),
          interviewId: interview.interviewId,
          videoQuestionId: question.videoQuestionId
    ...
    console.log("Seeding done with 3 users and multiple interviews!");
}
