...
import { Search, MapPin, Calendar, DollarSign, Building, Clock, Heart } from "lucide-react";
import { toast } from "sonner";
import CVPicker from "./CVPicker";

export default function JobSearch({ user, accessToken, onNavigate }) {
  const location = useLocation();
  // If someone navigates here with a CV in router state, use it
  const initialCv = location.state?.selectedCv || null;

  // Selected CV & modal visibility
  const [selectedCv, setSelectedCv] = useState(initialCv);
  const [showPicker, setShowPicker] = useState(!initialCv); // open on first load if none selected
  ...
  const [typeFilter, setTypeFilter] = useState(""); // empty string => placeholder
  const [levelFilter, setLevelFilter] = useState(""); // empty string => placeholder
  ...
  return (
    <div className={`flex min-h-screen bg-background ${showPicker ? "overflow-hidden" : ""}`}>
      {/* Glass modal */}
      <CVPicker
        open={showPicker}
      ...
      {/* Page content (blur softly when modal is open) */}
      <div className={`${showPicker ? "pointer-events-none select-none blur-sm" : ""} flex-1 flex`}>
        {/* Filters Sidebar */}
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Button
                ...
                title={selectedCv ? `Current: ${selectedCv.name}` : "No CV selected"}
              >
                {selectedCv ? "Change CV" : "Select a CV"}
              </Button>
            </div>
            ...
            {/* Job Type */}
            <div className="space-y-2">
              <label className="text-sm font-medium mt-2">Job Type</label>
            ...
            <Button
              ...
              onClick={() => {
                setSearchQuery("");
                setLocationFilter("");
                setTypeFilter(""); // back to placeholder
                setLevelFilter(""); // back to placeholder
                setRemoteOnly(false);
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  {selectedCv ? (
                    <Badge variant="secondary" title={selectedCv.name}>
                      Using CV: {selectedCv.name.length > 28 ? selectedCv.name.slice(0, 28) + "…" : selectedCv.name}
                    </Badge>
                  ) : null}
                </div>
            ...
            {selectedJob ? (
                  ...
                  <div>
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button onClick={() => applyToJob(selectedJob.id)} disabled={selectedJob.applied}>
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      <Badge className={getLevelColor(selectedJob.level)}>{selectedJob.level}</Badge>
                      <Badge className={getJobTypeColor(selectedJob.type)}>{selectedJob.type}</Badge>
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  <h3 className="text-lg font-medium text-muted-foreground mb-2">Select a job to view details</h3>
                  <p className="text-muted-foreground">Choose a job from the list to see full details and apply</p>
                </div>
              </div>
            )}
          </div>
